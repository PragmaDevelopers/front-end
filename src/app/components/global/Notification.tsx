...
export default function Notification({isShow}:{isShow:boolean}) {
    ...
    useEffect(()=>{
        get_notifications(undefined,userValue.token,(response)=>response.json().then((dbNotifications:NotificationProps[])=>{
            setNotifications(dbNotifications);
            console.log(dbNotifications)
        }));
        console.log("Foi")
        dayjs().format("yyyy-MM-dd'T'HH:mm:ss.SSSX");
    },[isShow])
                                ...
                                <div key={notification.id} className="w-full p-2 h-16 relative flex flex-col justify-start items-start">
                                    <div className="relative w-full h-fit flex flex-row items-center justify-start">
                                        ...
                                        <div className="flex flex-row absolute top-1 right-1 w-fit h-fit p-0.5">
                                            {/* <h2 className="text-neutral-400 font-xs">Há {dayjs(notification.registrationDate).toNow()}.</h2> */}
